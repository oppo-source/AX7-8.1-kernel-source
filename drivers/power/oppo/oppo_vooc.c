...
#define VOOC_NOTIFY_VAL6                  0x56
#define VOOC_NOTIFY_VAL7                    0x59
#define VOOC_NOTIFY_VAL8                        0x5c
#define VOOC_NOTIFY_VAL9                0x5b
#define VOOC_NOTIFY_VAL10                    0x5d
...
static void oppo_vooc_fastchg_func(struct work_struct *work)
{
		...
		ret_info = 0x2;
                ...
                del_timer(&chip->watchdog);
                ret_info = 0x2;
        } else if (data == VOOC_NOTIFY_VAL10) {
...
out:
        ...
        if (data == VOOC_NOTIFY_VAL4 || data == VOOC_NOTIFY_VAL7) {
        ...
        if ((data == VOOC_NOTIFY_VAL2 || (data_err && !phone_mcu_updated) || data == VOOC_NOTIFY_VAL10)
                && (chip->fastchg_dummy_started == false)) {
                oppo_chg_set_charger_type_unknown();
                oppo_chg_wake_update_work();
        } else if (data == VOOC_NOTIFY_VAL4 || data == VOOC_NOTIFY_VAL8 || data == VOOC_NOTIFY_VAL7
