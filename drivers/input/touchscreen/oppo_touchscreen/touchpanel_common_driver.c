...
static void tp_touch_down(struct touchpanel_data *ts, struct point_info points, int touch_report_num, int id)
{
...
#endif
    {
        ...
        if (ts->smart_gesture_support && (points.x > ts->touch_major_limit.width_range) && (points.x < ts->resolution_info.max_x - ts->touch_major_limit.width_range) &&\
    ...
    if(ts->fingerprint_underscreen_support && ts->fp_enable && ts->ts_ops->screenon_fingerprint_info) {
...
static void tp_touch_up(struct touchpanel_data *ts)
{
    ...
    if(ts->fingerprint_underscreen_support && ts->fp_enable && ts->ts_ops->screenon_fingerprint_info) {
        ts->ts_ops->screenon_fingerprint_info(ts->chip_data, &fp_tpinfo);
        if(fp_tpinfo.touch_state == FINGERPRINT_UP_DETECT) {
            TPD_INFO("screen on up : 0x%x\n", fp_tpinfo.touch_state);
            ts->fp_info.touch_state = 0;
            ts->fp_info.area_rate = 0;
            opticalfp_irq_handler(&ts->fp_info);
        }
    }
}
...
static void tp_work_func(struct touchpanel_data *ts)
{
    ...
    if (CHK_BIT(cur_event, IRQ_TOUCH) || CHK_BIT(cur_event, IRQ_BTN_KEY) || CHK_BIT(cur_event, IRQ_FW_HEALTH) || CHK_BIT(cur_event, IRQ_FACE_STATE)) {
...
static ssize_t proc_dir_control_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (!ts || !ts->ts_ops->set_touch_direction)
        return count;
