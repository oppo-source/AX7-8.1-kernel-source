...
wcnss_trigger_config(struct platform_device *pdev)
{
	...
	if (has_autodetect_xo == WCNSS_CONFIG_UNSPECIFIED && has_pronto_hw) {
		...
		//#ifdef VENDOR_EDIT
		//zhang liqun@PSW.CN.WiFi.Production.1366400, 2018/05/02,
		//Add for : set has_autodetect_xo according modemVersion
		project = get_project();
		modemVersion = get_Modem_Version();
		pr_err("%s: modemVersion= %d,project=%d\n", __func__, modemVersion,project);
		if(18031 == project || 18032 == project || 18301 == project){
			if(8 == modemVersion || 9 == modemVersion || 10 == modemVersion || 15 == modemVersion || 4 == modemVersion){
				has_autodetect_xo = 1;
			}else{
				has_autodetect_xo = 0;
			}
		}else if(18171 == project || 18172 == project || 18571 == project){
			if(63 == modemVersion || 59 == modemVersion){
				has_autodetect_xo = 1;
			}else{
				has_autodetect_xo = 0;
			}
		}else{
			pr_err("%s: project is wrong !!!\n", __func__);
		}
		pr_err("%s: has_autodetect_xo= %d\n", __func__, has_autodetect_xo);
		//#endif /* VENDOR_EDIT */
	}
